...
class AttModel(CaptionModel):
    def __init__(self, opt): #, tokenizer):
        ...
        self.max_seq_length = opt.max_length
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None,opt={}):
        ...
        sample_method = args.get('sample_method', 'greedy')
        beam_size = args.get('beam_size', 1)
        temperature = args.get('temperature', 1.0)
        sample_n = int(args.get('sample_n', 1))
        group_size = args.get('group_size', 1)
        output_logsoftmax = args.get('output_logsoftmax', 1)
        decoding_constraint = args.get('decoding_constraint', 0)
        block_trigrams = args.get('block_trigrams', 0)
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, att_masks, args)
        if group_size > 1:
            return self._diverse_sample(fc_feats, att_feats, att_masks, args)
