...
class Transformer(nn.Module):
    def __init__(self, encoder, decoder, src_embed, tgt_embed, rm, generator):
    ...
    def decode(self, hidden_states, src_mask, tgt, tgt_mask):
        ...
        #memory = self.rm.init_memory(50).to(hidden_states)
...
class EncoderDecoder(AttModel):
    ...
    def make_model(self, tgt_vocab):
        ...
        model = Transformer(
            ...
            Decoder(
                DecoderLayer(self.d_model, c(attn), c(attn), c(ff), self.dropout, self.rm_num_slots, self.rm_d_model),self.num_layers),
            lambda x: x,
    ...
    def __init__(self, opts): #, tokenizer):
        super(EncoderDecoder, self).__init__(opts) #, tokenizer)
        self.opts = opts
        '''
        ...
        '''
        self.num_layers = getattr(opts,'num_layers', 3)
        self.d_model = getattr(opts,'d_model',512)
        self.d_ff = getattr(opts,'d_ff',512)
        self.num_heads = getattr(opts,'num_heads',8)
        self.dropout = getattr(opts,'dropout',0.1)
        self.rm_num_slots = getattr(opts,'rm_num_slots',3)
        self.rm_num_heads = getattr(opts,'rm_num_heads',8)
        self.rm_d_model = getattr(opts,'rm_d_model',512)
