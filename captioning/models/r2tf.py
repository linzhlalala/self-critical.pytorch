...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, src_attn, feed_forward, dropout, rm_num_slots, rm_d_model):
        super(DecoderLayer, self).__init__()
        self.size = size
        self.self_attn = self_attn
        self.src_attn = src_attn
        self.feed_forward = feed_forward
        self.sublayer = clones(ConditionalSublayerConnection(size, dropout, rm_num_slots, rm_d_model), 3)
...
class r2tfModel(AttModel):

    def make_model(self, src_vocab, tgt_vocab, N_enc=3, N_dec=3, 
               d_model=512, d_ff=2048, h=8, dropout=0.1,rm_num_slots=3,rm_d_model=512,rm_num_heads=8):
        "Helper: Construct a model from hyperparameters."
        c = copy.deepcopy
        attn = MultiHeadedAttention(h, d_model, dropout)
        ff = PositionwiseFeedForward(d_model, d_ff, dropout)
        position = PositionalEncoding(d_model, dropout)
        rm = RelationalMemory(rm_num_slots, rm_d_model, rm_num_heads)
        model = EncoderDecoder(
            Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), N_enc),
            Decoder(DecoderLayer(d_model, c(attn), c(attn), c(ff), dropout, rm_num_slots, rm_d_model),N_dec),
    ...
    def __init__(self, opt):
        super(r2tfModel, self).__init__(opt)
        self.opt = opt
